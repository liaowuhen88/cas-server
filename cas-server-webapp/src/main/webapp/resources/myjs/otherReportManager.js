...
$(document).ready(function () {
    table = Tools.page({
        ...
        "ajaxUrl": window.base + "/physicalExamination/selectOtherReportData?rand=" + new Date().getTime(),
        "aoColumns": [
            {"sTitle": "序号", "aTargets": [0], "bSortable": false, "mData": "rownum"},
            {"sTitle": "报告编号", "aTargets": [1], "bSortable": false, "mData": "screening_num"},
            {"sTitle": "套餐名称", "aTargets": [2], "bSortable": false, "mData": "packagename"},
            {"sTitle": "报告公司", "aTargets": [3], "bSortable": false, "mData": "companyname"},
...
function initSelectData() {
    var url = window.base + '/physicalExamination/findReportJYInfo';
...
function showOtherInfo() {
    ...
    var url = window.base + '/physicalExamination/findOtherReportInfo/' + package_id;
    $.post(url, {}, function (result) {
        if (result.success) {
            var list = result.data;
...
function editClick(id) {
    ...
    $.get(window.base + "/physicalExamination/selectOtherPdfInfo", {'id': id}, function (msg) {
        var url = window.base + '/physicalExamination/findReportJYInfo'
        // 初始化套餐下拉框
        $.post(url, {}, function (result) {
            if (result.success) {
...
$("#addButton").click(function () {
    ...
    $.get(window.base + "/physicalExamination/findScreenNum", function (msg) {
        if (msg.data != null) {
            $("#screening_num").val(msg.data);
        }
    });
...
function pushClick(mobile, name, id) {
    ...
    if (r == true) {
        var url = window.base + "/physicalExamination/sendReportPush/" + mobile + "/" + name + "/" + id + "/2";
        ...
        $.ajax({
            ...
            success: function (responseData) {
                if (responseData.success) {
                    alert("发送成功");
                    location.href = "javascript:clickMenu('" + window.base + "/otherReportManager','otherReportManager')"
